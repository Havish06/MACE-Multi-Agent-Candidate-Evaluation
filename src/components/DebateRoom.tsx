...
export const DebateRoom: React.FC<DebateRoomProps> = ({
  ...
  const speakTurn = (index: number) => {
    if (!synthRef.current || index < 0 || index >= debateMessages.length) {
      setIsPlayingAudio(false);
      return;
    }

    synthRef.current.cancel();
    const msg = debateMessages[index];
    ...
    if (voices.length > 0) {
      if (msg.speaker === 'technical') {
        const fVoice = voices.find((v) => v.name.includes('Female') || v.name.includes('Zira') || v.name.includes('Samantha'));
        if (fVoice) utterance.voice = fVoice;
      } else if (msg.speaker === 'skeptic') {
        const dVoice = voices.find((v) => v.name.includes('David') || v.name.includes('George') || v.name.includes('Male'));
        if (dVoice) utterance.voice = dVoice;
      }
    }

    utterance.onend = () => {
      if (index + 1 < debateMessages.length) {
        speakTurn(index + 1);
      } else {
        setIsPlayingAudio(false);
      }
  ...
  const togglePlayAudio = () => {
    if (isPlayingAudio) {
      ...
      const nextIdx = currentMessageIndex >= 0 && currentMessageIndex < debateMessages.length ? currentMessageIndex : 0;
      speakTurn(nextIdx);
    }
  };
  ...
  const skipNextTurn = () => {
    ...
    if (nextIdx < debateMessages.length) {
      speakTurn(nextIdx);
    } else {
      if (synthRef.current) synthRef.current.cancel();
      setIsPlayingAudio(false);
      ...
      <div className="p-4 sm:p-5 rounded-xs bg-[#FFFFFF] border border-[#121212]/15 flex flex-col md:flex-row items-center justify-between gap-4 shadow-2xs">
        ...
        <div className="flex items-center gap-2.5 w-full md:w-auto justify-end">
          <button
            ...
            className={`inline-flex items-center gap-2 px-4 py-2 text-xs font-bold uppercase tracking-wider rounded-xs shadow-xs transition-all cursor-pointer ${
              isPlayingAudio
          ...
          <button
            ...
            disabled={currentMessageIndex >= debateMessages.length - 1}
          ...
          <button
            ...
            className="p-2 rounded-xs bg-[#FFFFFF] hover:bg-[#F4F1EA] text-[#121212] border border-[#121212]/20 transition-colors cursor-pointer shadow-2xs"
        ...
        <div className="p-5 rounded-xs bg-[#FFFFFF] border border-[#121212]/15 space-y-3.5 shadow-2xs">
          <div className="flex items-center justify-between pb-2 border-b border-[#121212]/10">
            <span className="text-xs font-bold uppercase tracking-[0.16em] text-[#121212] flex items-center gap-1.5">
              ...
              <span>Identified Key Committee Disagreements ({disputes.length})</span>
            </span>
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    >
                      {d.tensionLevel.toUpperCase()} TENSION
                    </span>
                  </div>
          ...
          return (
            <div
              ...
              className={`p-5 sm:p-6 rounded-xs border transition-all shadow-2xs ${
                isCurrentlySpeaking
                  ? 'bg-[#FAF0E6]/50 border-[#D94F33] ring-1 ring-[#D94F33]'
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mb-3.5 pb-2.5 border-b border-[#121212]/10">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="p-1 text-[#57534E] hover:text-[#D94F33] rounded hover:bg-[#F4F1EA] transition-colors cursor-pointer"
